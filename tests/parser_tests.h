...
#ifndef GITHUB_3081_REPO_GROUP_NEWTON_PROJECT_TESTS_PARSER_TESTS_H_
#define GITHUB_3081_REPO_GROUP_NEWTON_PROJECT_TESTS_PARSER_TESTS_H_
...
class ParserTestSuite : public CxxTest::TestSuite {
 public:
  ...
  void test_parse_bad_syntax() {
    const char *text =
        scanner::ReadInputFromFile("./samples/bad_syntax_good_tokens.dsl");
    TS_ASSERT(text);
    parser::ParseResult pr = p->Parse(text);
    TS_ASSERT(!pr.ok());
  }

  void test_parse_sample_1() {
    const char *filename = "./samples/sample_1.dsl";
    const char *text = scanner::ReadInputFromFile(filename);
    TS_ASSERT(text);
    parser::ParseResult pr = p->Parse(text);
...
#endif  // GITHUB_3081_REPO_GROUP_NEWTON_PROJECT_TESTS_PARSER_TESTS_H_
